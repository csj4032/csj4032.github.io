...
### Volatile 변수
* Volatile 변수로 약간 다른 형태의 좀더 약한 동기화 기능을 제공
* Volatile 선언된 변수의 값을 바꿨을 때 다른 스레드에서 항상 최신 값을 읽어갈 수 있도록 해줌

### 재진입성
* 스레드가 다른 스레드가 가진 락을 요청하면 해당 스레드는 대기 상태로 들어감
